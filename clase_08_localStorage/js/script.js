...
window.onload = function()
{
		...
		for(var i = 0; i < listadoPersonas.length; i++)
		{
			...
			if(muestra)
			{
				...
				nom_div("e_" + i).addEventListener('click', function(event)
				{
					...
					//Muestra en los campos los datos para ser editados.					
					MuestraUsuarioEditar(ind);
					Actualizar=true;
				});
				//Eliminar...
				nom_div("d_" + i).addEventListener('click', function(event)
				{
					var indE = event.target.id.split("_")[1];
					var idUser = listadoPersonas[indE].identificacion;
					if(confirm("¿Está segur@ de realizar está acción?"))
					{
						indE = buscaIndice(idUser);
						
						//Elimina el usuario en la posicion IndE..
						listadoPersonas.splice(indE,1);
	...
	//Valida que el usuario a actualizar no tenga el id de otro usuario.
	function existeUsuarioActulizar(id,email){
		...
		var existe=0;
			...
			if(listadoPersonas[i].identificacion === id || listadoPersonas[i].email.toLowerCase() === email.toLowerCase())
			{
				if(i==ind){
					existe = 0;
					...
					listadoPersonas[i].identificacion === id ? existe = 1 : existe = 2;
	...
	nom_div("guarda").addEventListener('click', function(event)
	{
		...
		if(Actualizar){
			//Funcion para editar usuario si Actualizar es True
			EditarUsuario();
	...
	//Funcion para esditar usuario una vez de a dado clic en guardar
	function EditarUsuario(){
		...
		if(correcto)
		{
			var existeDatos = existeUsuarioActulizar(valores[0],valores[3]);
			...
			if(existeDatos === 0) //No existe...
			{
				if(ValidaEmail(valores[3]))
				{
					listadoPersonas[ind].identificacion = valores[0];
					listadoPersonas[ind].primernombre = valores[1];
					listadoPersonas[ind].primerapellido = valores[2];
					listadoPersonas[ind].email = valores[3];
					listadoPersonas[ind].fechanacimiento = valores[4];
					localStorage.setItem("listado", JSON.stringify(listadoPersonas));
					imprimeUsuarios();
					limpiarCampos();
					ind="";
...
//Funcion que muestra los datos de usuario en los campos para ser editados	

function MuestraUsuarioEditar(id){

			nom_div(elementos[0]).value = listadoPersonas[id].identificacion;
			nom_div(elementos[1]).value = listadoPersonas[id].primernombre;
			nom_div(elementos[2]).value = listadoPersonas[id].primerapellido;
			nom_div(elementos[3]).value = listadoPersonas[id].email;
			nom_div(elementos[4]).value = listadoPersonas[id].fechanacimiento;
